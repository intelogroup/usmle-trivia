...
class AccessibilityAuditor {
  ...
  async testPage(url, pageName, requiresAuth = false) {
    ...
    try {
      ...
      await this.page.screenshot({ 
        path: `/root/repo/accessibility-screenshots/${pageName.replace(/\s+/g, '-')}-audit.png`,
        fullPage: true 
      });
  ...
  async testKeyboardNavigation(pageResults) {
    ...
    try {
      ...
      let tabCount = 0;
      const maxTabs = Math.min(focusableElements.length * 2, 25); // Prevent infinite loops
      ...
      images.forEach((img, index) => {
        if (!img.alt && !img.ariaLabel && img.role !== 'presentation') {
          issues.push({
            type: 'WCAG_1.1.1',
            severity: 'high',
            element: `img[src="${img.src.substring(img.src.lastIndexOf('/') + 1)}"]`,
      ...
      await this.page.addScriptTag({
        content: `
          ...
          window.checkColorContrast = () => {
            ...
            return issues.slice(0, 10); // Limit to prevent overwhelm
          };
        `
      });
  ...
  async generateReport() {
    ...
    this.results.summary.overallScore = totalIssues > 0 ? Math.max(0, 100 - (totalIssues * 2)) : 100;
